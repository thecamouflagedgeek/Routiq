...
const DRIVER_RISK: Record<number, RiskLevel> = {
  0: 'SAFE',
  1: 'MODERATE',
  2: 'HIGH',
  3: 'CRITICAL',
...
function ScorePill({ score, risk }: { score: number; risk: RiskLevel }) {
  ...
  return (
    <div
      ...
      style={{ background: `${meta.color}18`, border: `2px solid ${meta.color}`, color: meta.color }}
    >
...
function RiskChip({ level }: { level: RiskLevel }) {
  ...
  return (
    ...
    >
      <span className="h-1.5 w-1.5 rounded-full pulse-dot" style={{ background: meta.color }} />
      {meta.label}
    </span>
  )
}
...
function TopFactors({ segment }: { segment: Segment }) {
  ...
  if (!top.length) return <p className="text-[10px]" style={{ color: 'var(--text-3)' }}>No major risk factors.</p>
  return (
    <ul className="space-y-1">
      {top.map((e) => (
        <li key={e.factor} className="flex items-start gap-1.5">
          <AlertTriangle size={9} className="mt-0.5 shrink-0" style={{ color: 'var(--orange)' }} />
          <span className="text-[10px] leading-snug" style={{ color: 'var(--text-2)' }}>{e.factor}</span>
        </li>
      ))}
    </ul>
  )
...
export function DriveHUD({
  ...
  const driverData = useMemo(() => {
    ...
    // Phase 1: fresh driver
    // Phase 2: mildly fatigued (mirrors fatigue escalation level 1)
    // Phase 3: elevated fatigue (mirrors escalation level 2+)
    const base = fatigueState.escalation_level
  ...
  }, [demoPhase, fatigueState.escalation_level])

  const roadRisk: RiskLevel = currentSegment?.risk_level ?? 'SAFE'
  const driverRisk: RiskLevel = DRIVER_RISK[fatigueState.escalation_level] ?? driverData.risk
  const contextualRisk = fuseRisk(roadRisk, driverRisk)
  const contextMeta = RISK_META[contextualRisk]
  const isIntervention = contextualRisk === 'HIGH' || contextualRisk === 'CRITICAL'
    ...
    <div className="pointer-events-none flex flex-col gap-2 w-full">
        ...
        >
          <div className="flex items-start gap-2.5">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-0.5 text-[11px] font-semibold leading-snug" style={{ color: 'var(--text)' }}>
      ...
      >
        ...
        <div className="flex items-center gap-2">
          {/* road */}
          <div className="flex-1 rounded-xl p-2" style={{ background: 'var(--bg-2)' }}>
            <div className="text-[9px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-4)' }}>Road</div>
            <RiskChip level={roadRisk} />
          </div>
          <ChevronRight size={12} style={{ color: 'var(--text-4)', flexShrink: 0 }} />
          {/* driver */}
          <div className="flex-1 rounded-xl p-2" style={{ background: 'var(--bg-2)' }}>
            <div className="text-[9px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-4)' }}>Driver</div>
          ...
          <div
            ...
            style={{ background: `${contextMeta.color}12`, border: `1px solid ${contextMeta.color}35` }}
          >
            <div className="text-[9px] font-semibold uppercase tracking-wider" style={{ color: contextMeta.color }}>Fused</div>
      ...
      <div
        className="pointer-events-auto rounded-2xl p-3"
        style={{ background: 'var(--surface)', border: `1.5px solid ${currentSegment.risk_color}30` }}
      >
        <div className="flex items-start gap-3">
          <ScorePill score={currentSegment.safety_score} risk={currentSegment.risk_level} />
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2">
              <Navigation size={10} style={{ color: currentSegment.risk_color }} />
              <span className="text-[9px] font-black uppercase tracking-widest" style={{ color: 'var(--text-3)' }}>
                Current Segment
              </span>
            </div>
            <div className="mt-0.5 truncate text-xs font-bold" style={{ color: 'var(--text)' }}>
              {currentSegment.name}
            </div>
            <div className="mt-1">
                ...
                <ul className="space-y-1">
                  {currentSegment.explanation.map((e) => (
                    <li key={e.factor} className="flex items-start gap-1.5">
                      <AlertTriangle size={9} className="mt-0.5 shrink-0" style={{ color: 'var(--orange)' }} />
                      <div>
                        <span className="text-[10px] font-semibold leading-snug" style={{ color: 'var(--text)' }}>{e.factor}</span>
                        <span className="ml-1 text-[9px]" style={{ color: 'var(--text-3)' }}>{e.detail}</span>
                      </div>
                    </li>
                  ))}
                </ul>
        ...
        {currentSegment.risk_level === 'SAFE' && (
          <div className="mt-2 flex items-center gap-1.5">
            ...
            <span className="text-[10px]" style={{ color: 'var(--text-3)' }}>No significant risk factors.</span>
          </div>
        )}
      </div>
      ...
      {nextSegment && (
        ...
        >
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-black uppercase tracking-widest" style={{ color: 'var(--text-3)' }}>
                  Upcoming — {(nextSegment.distance_km * 1000).toFixed(0)} m ahead
                </span>
              </div>
              <div className="mt-0.5 truncate text-[11px] font-semibold" style={{ color: 'var(--text-2)' }}>
                {nextSegment.name}
              </div>
              <div className="mt-1">
          ...
          {(nextSegment.risk_level === 'HIGH' || nextSegment.risk_level === 'CRITICAL') && (
            <div
              className="mt-2 rounded-xl p-2"
              style={{ background: `${nextSegment.risk_color}10`, border: `1px solid ${nextSegment.risk_color}25` }}
            >
              <div className="flex items-center gap-1 mb-1">
                <ShieldAlert size={9} style={{ color: nextSegment.risk_color }} />
                <span className="text-[9px] font-bold uppercase tracking-wide" style={{ color: nextSegment.risk_color }}>
      ...
      >
        ...
        <div className="grid grid-cols-3 gap-1.5">
          <div className="rounded-xl p-2 text-center" style={{ background: 'var(--bg-2)' }}>
            <div className="text-base font-black" style={{ color: 'var(--text)' }}>{driverData.engagement}%</div>
            <div className="text-[9px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-4)' }}>Engagement</div>
          </div>
          <div className="rounded-xl p-2 text-center" style={{ background: 'var(--bg-2)' }}>
            <div className="text-base font-black" style={{ color: 'var(--text)' }}>{driverData.latency}s</div>
            <div className="text-[9px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-4)' }}>Latency</div>
          </div>
          <div className="rounded-xl p-2 text-center" style={{ background: 'var(--bg-2)' }}>
            <div className="text-base font-black" style={{ color: 'var(--text)' }}>{fatigueState.missed_responses}</div>
            <div className="text-[9px] font-semibold uppercase tracking-wider" style={{ color: 'var(--text-4)' }}>Missed</div>
          </div>
        </div>
        {/* progress bar */}
        <div className="mt-2">
          <div className="flex items-center justify-between mb-1">
            <span className="text-[9px] font-semibold" style={{ color: 'var(--text-4)' }}>Route progress</span>
            <span className="text-[9px] font-bold" style={{ color: 'var(--text-3)' }}>{Math.round(progress * 100)}%</span>
          </div>
          <div className="h-1.5 w-full overflow-hidden rounded-full" style={{ background: 'var(--bg-3)' }}>
            <div
              className="h-full rounded-full transition-all duration-500"
              style={{ width: `${progress * 100}%`, background: 'var(--orange)' }}
            />
          </div>
        </div>
      </div>
