...
import { DEFAULT_MAP_CENTER, DEV_LOCATION, EMERGENCY_SEARCH_RADIUS_KM } from '../config'
...
function distanceToRouteMeters(pos: { lat: number; lon: number }, geometry: LatLng[]): number {
  const R = 6371000
  ...
  for (let i = 0; i < geometry.length - 1; i++) {
    min = Math.min(min, distToSegment({ x: 0, y: 0 }, toLocal(geometry[i]), toLocal(geometry[i + 1])))
  }
  return min
}
...
export function Emergency({ onGoDashboard }: { onGoDashboard: () => void }) {
  ...
  const location = geo.position ?? DEV_LOCATION ?? { lat: DEFAULT_MAP_CENTER[0], lon: DEFAULT_MAP_CENTER[1] }
  ...
  const resolveLocation = useCallback(async (): Promise<{ lat: number; lon: number } | null> => {
    ...
    setLocationError('Unable to access your current location. Please enable location permissions.')
    return null
  }, [geo])

  /** Request the OSRM navigation route to the selected hospital. */
  const startNavigation = useCallback(async (loc: { lat: number; lon: number }, hospital: { lat: number; lon: number; id: string }) => {
    setRouting(true)
    setRouteError(null)
    lastRecalc.current = Date.now()
    try {
      const route = await api.getEmergencyRoute([loc.lat, loc.lon], [hospital.lat, hospital.lon], hospital.id)
      setNavRoute(route)
    } catch {
      // Keep the last good route on the map; never claim a route we don't have.
      setRouteError('Unable to compute the driving route right now. Follow the map to the hospital.')
    } finally {
      setRouting(false)
    }
  }, [])

  const activate = useCallback(async (radiusKm?: number) => {
    setActivating(true)
  ...
  const expandRadius = useCallback(() => {
    const bigger = Math.min(MAX_SEARCH_RADIUS_KM, Math.round(searchRadius * 1.67))
  ...
  useEffect(() => {
    ...
    const now = Date.now()
    if (off > ROUTE_DEVIATION_M && !recalcInFlight.current && now - lastRecalc.current > ROUTE_RECALC_COOLDOWN_MS) {
  ...
  const nextStep = useMemo(() => {
    ...
    return steps.find((s) => s.distance_m >= 30 && !s.instruction.toLowerCase().includes('arrive')) ?? null
  }, [navRoute])
  const nextInstruction = nextStep?.instruction ?? 'Follow the highlighted route'
    ...
    <div className="min-h-screen bg-neutral-50 pb-16">
      <div className="mx-auto mt-20 max-w-6xl px-4">
          ...
          <div className="relative h-[420px] overflow-hidden rounded-2xl border border-neutral-200 shadow-sm lg:h-[560px]">
            ...
            {mode === 'idle' && (
              <div className="absolute left-3 top-3 z-[1000] rounded-xl bg-white/95 px-3 py-2 text-xs font-semibold text-neutral-600 shadow">
                ...
                Your location{geo.position ? '' : usingDevLocation ? ' (dev override)' : ' (no GPS fix)'}
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            {mode === 'idle' && (
              <section className="rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
                ...
                <h2 className="mt-1.5 text-lg font-bold text-neutral-900">Simulate a collision</h2>
                <p className="mt-1 text-sm leading-relaxed text-neutral-500">
                  In a real vehicle, accelerometer spikes would trigger this. For the demo, simulate it — then
                  activate emergency response in one tap.
                </p>
                ...
                <p className="mt-3 text-[10px] leading-relaxed text-neutral-400">
                  Pipeline: sensor spike → velocity change → potential crash → driver confirmation → real GPS →
                  OpenStreetMap hospitals → OSRM ETA ranking → navigation.
                </p>
              </section>
            )}
              ...
              <>
                ...
                <section className="rounded-2xl border border-red-200 bg-white p-5 text-center shadow-sm">
                  <div className="relative mx-auto h-[150px] w-[150px]">
                    <svg width={150} height={150} className="-rotate-90">
                      <circle cx={75} cy={75} r={62} fill="none" stroke="#fee2e2" strokeWidth={9} />
                      <circle
                        cx={75}
                        cy={75}
                        r={62}
                    ...
                    <div className="absolute inset-0 flex flex-col items-center justify-center">
                      <span className="text-[10px] font-bold uppercase tracking-widest text-red-400">Response active</span>
                      <span className="text-3xl font-extrabold tabular-nums text-neutral-900">
                        {String(Math.floor(ring.remaining / 60)).padStart(2, '0')}:
                        {String(ring.remaining % 60).padStart(2, '0')}
                      </span>
                      <span className="text-[10px] text-neutral-400">until ETA estimate refreshes</span>
                    </div>
                  </div>
                  <div className="mt-2 rounded-xl bg-neutral-50 px-3 py-2 text-left">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-neutral-500">Fastest reachable hospital</span>
                      <span className="font-bold text-neutral-900">{topHospitalName}</span>
                    </div>
                    <div className="mt-1 flex items-center justify-between text-xs">
                      <span className="text-neutral-500">Estimated road ETA</span>
                      <span className="text-lg font-extrabold text-red-500">
                        {topHospital?.eta_min != null ? `${topHospital.eta_min} min` : '—'}
                      </span>
                    </div>
                  </div>
                </section>
                ...
                {navRoute && (
                  <section className="rounded-2xl border border-red-200 bg-white p-5 shadow-sm">
                    ...
                    <div className="mt-1 flex items-center justify-between gap-2">
                      <h2 className="text-lg font-extrabold tracking-tight text-neutral-900">🏥 {topHospitalName}</h2>
                      {routing && <Loader2 size={14} className="animate-spin text-red-500" />}
                    </div>
                    <div className="mt-2 flex items-center gap-6">
                      <div>
                        ...
                        <div className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">ETA</div>
                      </div>
                      <div>
                        <div className="text-2xl font-extrabold tabular-nums text-neutral-900">{navRoute.distance_km} km</div>
                        <div className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">distance</div>
                      </div>
                    </div>
                    <div className="mt-3 rounded-xl bg-red-50 px-3 py-2.5">
                      <div className="text-[9px] font-bold uppercase tracking-widest text-red-400">Next</div>
                      <div className="mt-0.5 text-sm font-bold text-neutral-900">{nextInstruction}</div>
                      {nextStep && <div className="mt-0.5 text-xs font-semibold text-neutral-500">{formatDistance(nextStep.distance_m)}</div>}
                    </div>
                    <div className="mt-2 flex flex-wrap items-center gap-x-2 gap-y-1 text-[10px] text-neutral-400">
                      <span
                        className={`rounded-full px-2 py-0.5 font-bold tracking-wider ${
                          navRoute.source === 'live' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
                        }`}
                      >
                        {navRoute.source === 'live' ? '● LIVE' : '○ DEMO'} · {navRoute.provider.toUpperCase()}
                      </span>
                ...
                <section className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
                  ...
                  <div className="mt-2 grid grid-cols-2 gap-2">
                    ...
                    >
                      {sharing === 'copied' ? <ClipboardCopy size={15} /> : sharing === 'ok' ? <CheckCircle2 size={15} /> : <Share2 size={15} />}
        ...
        {mode === 'active' && emergency && (
          <section className="mt-5 rounded-2xl border border-neutral-200 bg-white p-5 shadow-sm">
            <div className="mb-3 flex items-center justify-between">
              <div>
                <SectionLabel>Nearest reachable hospitals — ranked by road ETA</SectionLabel>
                <p className="mt-0.5 text-[11px] text-neutral-400">
                  Discovered live from <span className="font-semibold text-neutral-600">OpenStreetMap</span> around your
                  GPS position · ranked by real <span className="font-semibold text-neutral-600">OSRM</span> road ETA.
                  The fastest reachable care wins.
                </p>
              </div>
            </div>
            {emergency.hospitals.length === 0 ? (
              <div className="rounded-xl border border-amber-200 bg-amber-50 px-4 py-5 text-center">
                ...
                <p className="mt-0.5 text-xs text-amber-700">Try widening the search around your location.</p>
                <button
                ...
                >
                  {activating ? 'Searching…' : `Expand search radius to ${Math.min(MAX_SEARCH_RADIUS_KM, Math.round(searchRadius * 1.67))} km`}
                </button>
              </div>
            ) : (
                ...
                {emergency.hospitals.map((h, i) => (
                  <li
                    ...
                    className={`flex items-center gap-3 rounded-xl border px-4 py-3 ${
                      i === 0 ? 'border-red-300 bg-red-50/60' : 'border-neutral-100 bg-neutral-50/60'
                    }`}
                  >
                    <span
                    ...
                    <div className="min-w-0 flex-1">
                      <div className="flex items-center gap-2">
                        <div className="truncate text-sm font-bold text-neutral-900">{h.name || 'Hospital'}</div>
                        {i === 0 && (
                      ...
                      <div className="mt-0.5 flex flex-wrap items-center gap-x-3 gap-y-0.5 text-[11px] text-neutral-500">
                        {h.eta_min != null ? (
                          <>
                            <span className="text-sm font-extrabold text-neutral-900">{h.eta_min} min</span>
                            <span>ETA</span>
                          </>
                        ) : (
                          <span className="text-xs font-semibold text-amber-600">Driving time unavailable</span>
                        )}
                        <span>·</span>
                        <span>{h.distance_km} km</span>
                        <span
                          className={`rounded-full px-1.5 py-0.5 text-[9px] font-bold uppercase tracking-wider ${
                            h.eta_source === 'live' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'
                          }`}
                        >
                          {h.eta_source === 'live' ? 'Live road ETA' : h.eta_source === 'estimated' ? 'Estimated' : 'No route'}
                        </span>
                      </div>
                    </div>
                    {h.phone && (
            ...
            <p className="mt-3 text-[10px] text-neutral-400">
              Hospital data: <span className="font-semibold">OpenStreetMap</span> · Road ETAs:{' '}
              <span className="font-semibold">OSRM</span>. Hospitals OSRM couldn’t route show “Driving time
              unavailable” instead of a made-up ETA.
            </p>
          </section>
        )}
        ...
        <p className="mt-4 flex items-start gap-1.5 text-[10px] leading-relaxed text-neutral-400">
          ...
          Hackathon prototype: ETAs and crash detection are estimates, not guarantees. In a real emergency, call your
      ...
      {mode === 'crash' && (
        <div className="fixed inset-0 z-[1500] flex items-center justify-center bg-neutral-950/70 p-4 backdrop-blur-sm">
          <div className="alert-flash w-full max-w-md rounded-3xl border-2 border-red-400 bg-white p-8 text-center shadow-2xl">
            ...
            <h2 className="text-2xl font-extrabold tracking-tight text-neutral-900">Potential collision detected</h2>
            <p className="mt-2 text-sm leading-relaxed text-neutral-500">
              Sudden deceleration and acceleration spike detected. Are you okay?
            </p>
            {locationError && (
              <div className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-sm font-semibold text-red-700">{locationError}</div>
            )}
            {hospitalError && (
              <div className="mt-3 rounded-xl bg-red-50 px-3 py-2 text-sm font-semibold text-red-700">{hospitalError}</div>
            )}
            {recovered && (
            ...
            <div className="mt-5 flex flex-col gap-2">
              ...
              <PillButton variant="red" onClick={() => void activate()} disabled={activating}>
                {activating ? <Loader2 size={15} className="animate-spin" /> : <Siren size={15} />}
                ACTIVATE EMERGENCY RESPONSE
              </PillButton>
            </div>
            {!geo.position && !DEV_LOCATION && (
              <p className="mt-3 text-[10px] leading-relaxed text-neutral-400">
                No GPS fix yet. We'll request your real location on activation — enable location permissions when
                prompted. (Developers: set <code className="font-mono">VITE_DEV_LOCATION=&quot;lat,lon&quot;</code> to
                test without GPS.)
              </p>
            )}
          </div>
      ...
      {activating && (
        <div className="fixed inset-0 z-[1600] flex items-center justify-center bg-white/60 backdrop-blur-sm">
          <div className="flex items-center gap-3 rounded-2xl border border-neutral-200 bg-white px-6 py-4 shadow-xl">
            ...
            <span className="text-sm font-semibold text-neutral-700">Finding nearby hospitals on OpenStreetMap…</span>
          </div>
        </div>
      )}
    </div>
