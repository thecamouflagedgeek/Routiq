  ...
  return (
    <div className="flex-1 rounded-2xl border border-neutral-200 bg-white px-4 py-3 text-center shadow-sm">
      ...
      <div className="mt-0.5 flex items-baseline justify-center gap-1">
        <span className="text-2xl font-black tabular-nums tracking-tight" style={{ color }}>
          {value}
        </span>
        {trend === 'down' && <span className="text-sm font-black text-red-500">↓</span>}
        {trend === 'up' && <span className="text-sm font-black text-red-500">↑</span>}
      </div>
      {sub && <div className="text-[10px] font-medium text-neutral-400">{sub}</div>}
    </div>
  )
}
    ...
    <div className="flex min-h-screen flex-col bg-neutral-50 pt-20 sm:pt-24">
      ...
      {active && !d.audio_healthy && (
        <div className="mx-auto w-full max-w-6xl px-4 pt-2">
          <div className="flex items-center gap-2 rounded-xl border border-red-200 bg-red-50 px-4 py-2.5 text-xs font-medium text-red-700">
            ...
            <span>
              <b>MICROPHONE UNAVAILABLE</b> — silence will not raise your fatigue
              estimate while the mic is down. Use the simulate buttons below to
              keep demonstrating.
            </span>
          </div>
        </div>
      ...
      <div className="mx-auto mt-4 grid w-full max-w-6xl flex-1 gap-5 px-4 pb-8 lg:grid-cols-[1fr_340px] lg:min-h-0">
          ...
          <div className="flex min-h-0 flex-1 flex-col items-center justify-center p-6 text-center">
            ...
            {active && (
              <div className="flex w-full max-w-lg flex-col items-center gap-4">
                      ...
                      {f.listening && (
                        <>
                          ...
                          <span
                            ...
                            style={{ width: 92, height: 92, animation: 'ping 1.1s cubic-bezier(0,0,0.2,1) infinite' }}
                          />
                        </>
                      )}
                      <button
                        ...
                        style={{ background: f.listening ? '#f97316' : '#1c1c1c', transform: f.listening ? 'scale(1.08)' : 'scale(1)' }}
                      >
                        {f.listening ? <Mic size={26} className="text-white" /> : <MicOff size={22} className="text-neutral-500" />}
                      </button>
                    </div>

                    <p className={`-mt-1 text-sm font-semibold ${f.listening ? 'animate-pulse text-orange-500' : 'text-neutral-400'}`}>
                      {f.mode === 'demo'
                    ...
                    <div className="w-full">
                      ...
                      <div className="h-2.5 w-full overflow-hidden rounded-full bg-neutral-100">
                        <div
                          ...
                          style={{ width: `${pct}%`, backgroundColor: timerColor, transition: 'width 0.1s linear, background-color 0.4s ease' }}
                        />
                      </div>
                      <div className="mt-1.5 flex justify-between text-[10px] font-medium text-neutral-400">
                        ...
                        <span>max {f.maxWait}s</span>
                      </div>
                    </div>

                    {/* simulate controls — mic unavailable */}
                    {f.mode === 'live' && (!f.micSupported || !d.audio_healthy) && (
                ...
                {f.phase === 'paused' && <p className="text-sm text-neutral-400">Paused — monitoring on hold.</p>}
                {f.phase === 'intro' && <p className="animate-pulse text-sm font-semibold text-neutral-500">Warming up…</p>}
                      ...
                      {f.history.slice(-6).map((turn, i) => (
                        <div
                          ...
                          className={`flex gap-2 text-xs leading-relaxed ${
                            turn.role === 'routiq' ? 'text-neutral-700' : 'text-neutral-500'
                          }`}
                        >
                          <span
                ...
                {f.cooldownRemaining > 0.5 && !waiting && (quiet || analyzing) && (
                  <p className="text-[11px] font-medium text-neutral-400">
                    Next check-in in ~{Math.ceil(f.cooldownRemaining)}s ·{' '}
                    {state === 'NORMAL' ? 'quiet monitoring' : 'pacing interventions'}
                  </p>
                )}
              </div>
            )}
        ...
        <aside className="flex flex-col gap-4">
          ...
          <section className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
            ...
            <div className="mt-3 flex items-center justify-between">
              <div>
                ...
                <div className="text-[11px] text-neutral-400">{meta.description}</div>
              </div>
              <RiskBadge level={meta.riskLabel} />
            </div>
            <div className="mt-3 flex items-center justify-around">
              <div className="text-center">
                <div className="text-3xl font-black tabular-nums" style={{ color: meta.color }}>
                  {engagement}%
                </div>
                <div className="text-[10px] font-semibold uppercase tracking-widest text-neutral-400">Engagement</div>
              </div>
            ...
            {d.baseline_latency_ms != null && (
              <div className="mt-3 flex items-center justify-center gap-1.5 rounded-xl bg-neutral-50 py-2 text-[11px] font-semibold text-neutral-500">
                ...
                Personal baseline {(d.baseline_latency_ms / 1000).toFixed(1)}s · {d.baseline_samples > 0 ? `${d.baseline_samples} samples` : 'learning…'}
              </div>
            )}
            <div className="mt-3 grid grid-cols-3 gap-2 text-center">
              <div className="rounded-xl bg-neutral-50 py-2">
                <div className="text-lg font-extrabold text-neutral-900">{d.recent_delayed_responses}</div>
                <div className="text-[9px] font-semibold uppercase tracking-widest text-neutral-400">Delayed</div>
              </div>
              <div className="rounded-xl bg-neutral-50 py-2">
                <div className="text-lg font-extrabold text-neutral-900">{d.missed_responses}</div>
                <div className="text-[9px] font-semibold uppercase tracking-widest text-neutral-400">Missed</div>
              </div>
              <div className="rounded-xl bg-neutral-50 py-2">
                <div className="text-lg font-extrabold text-neutral-900">{d.interventions_triggered}</div>
                <div className="text-[9px] font-semibold uppercase tracking-widest text-neutral-400">Interventions</div>
              </div>
            </div>
          </section>
          ...
          <section className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
            ...
            {d.evidence.length === 0 ? (
              <p className="mt-2 text-xs text-neutral-400">Collecting interaction signals…</p>
            ) : (
              ...
              {ESCALATION_STEPS.map((s) => (
                <div
                  ...
                  className={`flex items-center gap-3 rounded-xl border px-3 py-2 transition-colors ${
                    state === s.state ? 'border-neutral-800 bg-neutral-50' : 'border-neutral-100'
                  }`}
                >
                  <span
                    className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[10px] font-extrabold text-white"
                    style={{ backgroundColor: state === s.state ? s.color : '#d4d4d4' }}
                  >
                    {ESCALATION_STEPS.indexOf(s) + 1}
                  </span>
                  <span className="min-w-0 flex-1">
                    <span className={`block text-xs font-bold ${state === s.state ? 'text-neutral-900' : 'text-neutral-500'}`}>
                      {s.label}
                    </span>
                    <span className="block truncate text-[10px] text-neutral-400">{s.desc}</span>
                  </span>
                  {state === s.state && <Gauge size={14} style={{ color: s.color }} />}
                </div>
              ))}
            </div>
          </section>
            ...
            <div className="mt-3 flex items-center justify-between">
              <div className="min-w-0">
                ...
                <div className="mt-0.5 truncate text-[10px] text-neutral-400">
                  {f.aiAvailable === null ? 'Probing AI availability…' : f.aiAvailable ? 'Groq conversational replies (falls back to scripted)' : 'Scripted assistant — AI unavailable'}
                </div>
              </div>
              <button
            ...
            <div className="mt-3 space-y-3">
              ...
              <div>
                <div className="flex items-center justify-between text-xs font-medium text-neutral-700">
                  ...
                  <span className="text-[10px] text-neutral-400">only after you ask</span>
                </div>
          ...
          {/* thresholds */}
          <section className="rounded-2xl border border-neutral-200 bg-white p-4 shadow-sm">
            <button onClick={() => setShowSettings((v) => !v)} className="flex w-full cursor-pointer items-center justify-between">
                ...
                {(
                  ...
                  <label key={key} className="rounded-xl bg-neutral-50 px-3 py-2">
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        f.updateThresholds({ ...f.thresholds, [key]: Number(e.target.value) })
                      }
      ...
      <div className="mx-auto flex max-w-6xl items-center gap-2 px-4 pb-6">
        ...
        <p className="text-[10px] leading-relaxed text-neutral-400">
          Bluetooth-ready: in a production vehicle this runs through car speakers
          and microphone via the audio transport. Here it uses your computer's
          mic and speakers. Audio failures never count toward fatigue.
        </p>
      </div>
        ...
        <div className="fixed inset-0 z-[1500] flex items-center justify-center bg-red-950/90 p-4 backdrop-blur-sm">
          <div className="alert-flash w-full max-w-md rounded-3xl border-2 border-red-400 bg-red-50 p-8 text-center">
            ...
            <p className="mt-2 text-sm leading-relaxed text-red-900/80">
              ...
              pull over at the next safe location as soon as it is safe to do so.
            </p>
            <div className="mt-3 rounded-xl bg-red-100/70 p-3 text-left text-[11px] font-medium text-red-800">
              <div className="mb-1 font-bold uppercase tracking-widest text-red-500">Evidence</div>
